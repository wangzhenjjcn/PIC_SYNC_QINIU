# AI上传同步文件001 同步客户端 (MVP)

本项目是基于 PySide6 的桌面应用，用于在本地指定文件夹与七牛云对象存储空间（Bucket）之间进行同步（服务器作为中转和对外访问端点）。
...
- 同步流程骨架：拉取/生成清单、计算差异、锁文件机制、上传/下载/删除（部分细节后续完善）
...
- 清单与锁文件固定位于服务器的 `__sync/` 目录下，不参与业务文件同步计算。
- 目前为 MVP 骨架，后续将逐步完善大文件分片、断点续传、冲突副本保留等细节。 